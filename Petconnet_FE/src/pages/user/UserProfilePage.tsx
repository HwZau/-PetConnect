...
const UserProfilePage = () => {
  ...
  const handleDeletePet = (petKey: string) => {
    const pet = user?.pets?.find((p) => {
      const pKey = p.petId || p._id || p.id;
      return pKey === petKey;
    });
    if (pet) {
      setPetToDelete({ petId: pet.petId || pet._id || pet.id || petKey, petName: pet.petName });
                    ...
                    <div className="grid md:grid-cols-2 gap-4">
                      {user?.pets && user.pets.length > 0 ? (
                        user.pets.map((pet, petIndex) => {
                          const petKey = pet.petId || pet._id || pet.id || `pet-${petIndex}`;
