...
router.post('/create', auth, async (req, res) => {
  try {
    ...
    // Supported payment methods (only MoMo and VNPAY)
    ...
    // Create proper QR format for MoMo and VNPAY
    // MoMo format: https://qr.momo.vn/..
    // For now, we'll create a data format that apps can parse
    const timestamp = Date.now();
    const qrData = method === 2 ? 
      // MoMo: Account|Amount|Description|OrderCode
      `${methodDetails.account}|${booking.totalAmount}|Thanh toan DV|${payment.orderCode}` :
      // VNPAY: Account|Amount|Description|OrderCode  
      `${methodDetails.account}|${booking.totalAmount}|Thanh toan DV|${payment.orderCode}`;

    console.log('Payment created successfully:', { paymentId: payment._id, method, amount: booking.totalAmount, qrData });
