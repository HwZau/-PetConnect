import React, { useEffect, useState, useCallback } from "react";
import { useParams, useNavigate } from "react-router-dom";
import adminService from "../../services/admin/adminService";
import CustomerModal from "../../components/admin/modal/CustomerModal";
import type { CustomerFormData } from "../../components/admin/modal/CustomerModal";
import { showSuccess, showError } from "../../utils/toastUtils";
import { useSettings } from "../../contexts/SettingsContext";
import { FaMapMarkerAlt, FaPhone, FaEnvelope, FaHome } from "react-icons/fa";

interface CustomerDetail {
  id: string;
  name: string;
  email: string;
  phoneNumber?: string;
  address?: string;
  role: string;
  avatarUrl?: string | null;
  subtitle?: string;
  status?: string;
  joinDate?: string;
  region?: string;
  petInfo?: string;
  bookingCount?: number;
  totalSpent?: string;
}

const CustomerDetailPage: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { theme } = useSettings();

  const [customer, setCustomer] = useState<CustomerDetail | null>(null);
  const [loading, setLoading] = useState(true);
  const [isEditOpen, setIsEditOpen] = useState(false);
  const [editInitial, setEditInitial] = useState<Partial<CustomerFormData> | undefined>(undefined);

  const loadCustomer = useCallback(async (forceId?: string) => {
    const uid = forceId || id;
    if (!uid) return;
    setLoading(true);
    try {
      const userResp = await adminService.getUserById(uid);
      if (userResp.success && userResp.data) {
        const data = userResp.data as any;
        setCustomer({
          id: String(data.id || ""),
          name: String(data.name || ""),
          email: String(data.email || ""),
          phoneNumber: String(data.phoneNumber || data.phone || "N/A"),
          address: String(data.address || data.subtitle || "N/A"),
          role: String(data.role || "Customer"),
          avatarUrl: (data.avatar || data.avatarUrl) as string | null,
          subtitle: data.subtitle as string | undefined,
          status: data.status as string | undefined,
          joinDate: data.joinDate as string | undefined,
          region: data.region as string | undefined,
          petInfo: data.petInfo as string | undefined,
          bookingCount: data.bookingCount as number | undefined,
          totalSpent: data.totalSpent as string | undefined,
        });
        return;
      }

      const resp = await adminService.getUserById(uid);
      if (resp.success && resp.data) {
        const data = resp.data as any;
        setCustomer({
          id: String(data.id || ""),
          name: String(data.name || ""),
          email: String(data.email || ""),
          phoneNumber: String(data.phoneNumber || data.phone || "N/A"),
          address: String(data.address || "N/A"),
          role: String(data.role || "Customer"),
          avatarUrl: (data.avatar || data.avatarUrl) as string | null,
          subtitle: data.subtitle as string | undefined,
          status: data.status as string | undefined,
          joinDate: data.joinDate as string | undefined,
          region: data.region as string | undefined,
          petInfo: data.petInfo as string | undefined,
          bookingCount: data.bookingCount as number | undefined,
          totalSpent: data.totalSpent as string | undefined,
        });
      } else {
        showError(resp.message || "Không thể tải hồ sơ khách hàng");
        setCustomer(null);
      }
    } catch (err) {
      console.error(err);
      showError("Lỗi khi lấy dữ liệu khách hàng");
      setCustomer(null);
    } finally {
      setLoading(false);
    }
  }, [id]);

  useEffect(() => { loadCustomer(); }, [id, loadCustomer]);

  const openEdit = () => {
    if (!customer) return;
    setEditInitial({
      name: customer.name,
      email: customer.email,
      phoneNumber: customer.phoneNumber,
      address: customer.address,
    });
    setIsEditOpen(true);
  };

  const handleEditSubmit = async (data: CustomerFormData) => {
    if (!customer) return;
    try {
      const payload: Partial<Record<string, unknown>> = {
        name: data.name,
        phoneNumber: data.phoneNumber,
        address: data.address,
        role: customer.role,
      };
      const res = await adminService.updateUser(customer.id, payload as any);
      if (res.success) {
        showSuccess('Cập nhật khách hàng thành công');
        setIsEditOpen(false);
        await loadCustomer(customer.id);
      } else {
        showError(res.error || res.message || 'Lỗi khi cập nhật');
      }
    } catch (err) {
      console.error(err);
      showError('Lỗi khi cập nhật khách hàng');
    }
  };

  if (loading) {
    return (
      <div className="p-8">
        <div className="animate-pulse space-y-4">
          <div className="h-6 bg-gray-200 rounded w-1/3" />
          <div className="h-40 bg-gray-200 rounded" />
        </div>
      </div>
    );
  }

  if (!customer) {
    return (
      <div className="p-8 text-center">
        <h2 className="text-2xl font-bold mb-2">Hồ sơ không tồn tại</h2>
        <p className="text-gray-500 mb-4">Hồ sơ khách hàng đã bị xóa hoặc không tồn tại.</p>
        <button onClick={() => navigate('/admin/customers')} className="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700">
          Quay lại Danh sách
        </button>
      </div>
    );
  }

  return (
    <div className={`p-8 ${theme === 'dark' ? 'bg-gray-900 text-gray-100' : 'bg-gray-50 text-gray-800'}`}>
      {/* Header */}
      <div className="flex items-start gap-6 mb-8">
        <img 
          src={customer.avatarUrl || 'https://i.pravatar.cc/150?u=' + customer.id} 
          alt={customer.name} 
          className="w-40 h-40 rounded-2xl object-cover shadow-lg" 
        />
        <div className="flex-1">
          <h1 className="text-4xl font-bold mb-2">{customer.name}</h1>
          <p className="text-gray-500 mb-4">{customer.subtitle || 'Khách hàng'}</p>
          
          <div className="flex items-center gap-3 mb-4 flex-wrap">
            {customer.status && (
              <span className={`inline-flex items-center gap-2 px-4 py-2 rounded-full font-semibold ${
                customer.status === 'Active' ? 'bg-green-100 text-green-800' :
                customer.status === 'VIP' ? 'bg-yellow-100 text-yellow-800' :
                'bg-red-100 text-red-800'
              }`}>
                {customer.status}
              </span>
            )}
            {customer.region && (
              <span className="inline-flex items-center gap-2 bg-blue-100 text-blue-800 px-4 py-2 rounded-full">
                <FaMapMarkerAlt /> {customer.region}
              </span>
            )}
          </div>
          
          <div className="grid grid-cols-2 gap-4 text-sm">
            {customer.bookingCount !== undefined && <div>Số lần đặt: <strong>{customer.bookingCount}</strong></div>}
            {customer.totalSpent && <div>Tổng chi tiêu: <strong>{customer.totalSpent}</strong></div>}
            {customer.petInfo && <div>Thú cưng: <strong>{customer.petInfo}</strong></div>}
            <div>Vai trò: <strong>{customer.role}</strong></div>
          </div>
        </div>
      </div>

      {/* Main Content */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Left Panel - Customer Info */}
        <div className="lg:col-span-2 space-y-6">
          {/* Activity/Stats */}
          <div className={`p-6 rounded-2xl shadow-md ${
            theme === 'dark' ? 'bg-gray-800' : 'bg-white'
          }`}>
            <h2 className="text-xl font-bold mb-4">Thông tin chi tiết</h2>
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className={`p-4 rounded-lg ${
                  theme === 'dark' ? 'bg-gray-700' : 'bg-gray-50'
                }`}>
                  <p className="text-xs text-gray-500 uppercase mb-1">Email</p>
                  <p className="font-semibold">{customer.email}</p>
                </div>
                <div className={`p-4 rounded-lg ${
                  theme === 'dark' ? 'bg-gray-700' : 'bg-gray-50'
                }`}>
                  <p className="text-xs text-gray-500 uppercase mb-1">Số điện thoại</p>
                  <p className="font-semibold">{customer.phoneNumber}</p>
                </div>
              </div>
              <div className={`p-4 rounded-lg ${
                theme === 'dark' ? 'bg-gray-700' : 'bg-gray-50'
              }`}>
                <p className="text-xs text-gray-500 uppercase mb-1">Địa chỉ</p>
                <p className="font-semibold">{customer.address}</p>
              </div>
              {customer.joinDate && (
                <div className={`p-4 rounded-lg ${
                  theme === 'dark' ? 'bg-gray-700' : 'bg-gray-50'
                }`}>
                  <p className="text-xs text-gray-500 uppercase mb-1">Ngày tham gia</p>
                  <p className="font-semibold">{new Date(customer.joinDate).toLocaleDateString('vi-VN')}</p>
                </div>
              )}
            </div>
          </div>
        </div>

        {/* Right Panel - Contact Info */}
        <div className={`p-6 rounded-2xl shadow-md h-fit ${
          theme === 'dark' ? 'bg-gray-800' : 'bg-white'
        }`}>
          <h3 className="text-xl font-bold mb-6">Thông tin liên hệ</h3>
          
          <div className="space-y-4">
            <div className="flex items-start gap-3">
              <FaEnvelope className="text-green-600 mt-1 flex-shrink-0" />
              <div>
                <p className="text-xs text-gray-500 uppercase">Email</p>
                <p className="font-semibold">{customer.email}</p>
              </div>
            </div>
            
            <div className="flex items-start gap-3">
              <FaPhone className="text-green-600 mt-1 flex-shrink-0" />
              <div>
                <p className="text-xs text-gray-500 uppercase">Số điện thoại</p>
                <p className="font-semibold">{customer.phoneNumber || 'N/A'}</p>
              </div>
            </div>
            
            <div className="flex items-start gap-3">
              <FaHome className="text-green-600 mt-1 flex-shrink-0" />
              <div>
                <p className="text-xs text-gray-500 uppercase">Địa chỉ</p>
                <p className="font-semibold">{customer.address || 'N/A'}</p>
              </div>
            </div>
          </div>

          <div className="mt-8 flex gap-2">
            <button 
              onClick={() => navigate('/admin/customers')} 
              className={`flex-1 px-4 py-2 rounded-lg font-semibold transition ${
                theme === 'dark' 
                  ? 'bg-gray-700 hover:bg-gray-600 text-gray-100' 
                  : 'bg-gray-200 hover:bg-gray-300 text-gray-800'
              }`}
            >
              Quay lại
            </button>
            <button 
              onClick={openEdit} 
              className="flex-1 px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg font-semibold transition"
            >
              Chỉnh sửa
            </button>
          </div>
        </div>
      </div>

      {/* Edit Modal */}
      <CustomerModal
        isOpen={isEditOpen}
        onClose={() => setIsEditOpen(false)}
        onSubmit={handleEditSubmit}
        initialData={editInitial}
        title="Chỉnh sửa Khách hàng"
        submitLabel="Lưu thay đổi"
      />
    </div>
  );
};

export default CustomerDetailPage;
