...
const FreelancerModal = ({ isOpen, onClose, onSubmit, services }: FreelancerModalProps) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleResetAndClose} title={t('Thêm Freelancer Mới','Add New Freelancer')}>
      <form onSubmit={handleSubmit}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-2">
          <div>
            ...
            <FormInput
              label={t('Chuyên môn','Specialty')}
              name="subtitle"
              placeholder={t('VD: Chuyên gia chăm sóc chó','e.g. Pet grooming expert')}
              icon={<AiOutlineStar />}
            ...
            <FormInput
              label={t('Kinh nghiệm','Experience')}
              name="experience"
              type="select"
              icon={<AiOutlineStar />}
              ...
              handleInputChange={handleInputChange}
            >
              <option value="">{t('Chọn kinh nghiệm','Select experience')}</option>
              <option value="1">{t('Dưới 1 năm','Under 1 year')}</option>
              <option value="1-3">{t('1-3 năm','1-3 years')}</option>
              <option value="3-5">{t('3-5 năm','3-5 years')}</option>
              <option value="5+">{t('Trên 5 năm','5+ years')}</option>
            </FormInput>
          </div>

          <div>
            <FormInput
              label={t('Loại dịch vụ','Service Type')}
              name="serviceType"
              type="select"
              icon={<AiOutlineUser />}
            ...
            >
              <option value="">{t('Chọn loại dịch vụ (Tự động điền giá)','Select service (auto-fill price)')}</option>
              {services.map(s => (
