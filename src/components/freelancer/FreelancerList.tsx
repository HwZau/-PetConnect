...
const FreelancerList: React.FC<FreelancerListProps> = ({ filters }) => {
  ...
  const displayFreelancers = freelancers.map((freelancer) => {
    const avgRating = freelancerService.calculateAverageRating(
      freelancer.reviewsReceived
    );
    const reviewCount = freelancer.reviewsReceived.length;

    return {
      id: freelancer.id,
      name: freelancer.name,
      avatar: freelancer.avatarUrl || "https://via.placeholder.com/150",
