...
export interface Post {
  ...
  authorId?: string; // Add authorId for ownership checks
  content: string;
  images?: string[];
  imageUrl?: string;
  timestamp: string;
...
const CommunityFeed: React.FC<CommunityFeedProps> = ({
  ...
  const handleAddPost = () => {
    ...
    onAddPost({
      author: "Bạn", // This will be overridden in CommunityPage with actual user name
      title: newPostTitle,
      caption: newPost,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-md border border-gray-100 overflow-hidden">
        {!safeIsAuthenticated ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600 mb-6 text-lg">Vui lòng đăng nhập để chia sẻ với cộng đồng</p>
            <button
              type="button"
              onClick={(e) => {
                e.preventDefault();
      ...
      {posts.map((post) => {
        ...
        // Check ownership by comparing user ID or name
        const isOwnPost = (currentUserId && post.authorId && String(currentUserId) === String(post.authorId)) ||
                         (user?.id && post.authorId && String(user.id) === String(post.authorId)) ||
                         (user?.name && post.author.name === user.name);
        const isEditing = editingPostId === String(post.id);
        console.log("[DEBUG] Post ownership check:", {
          postId: post.id,
