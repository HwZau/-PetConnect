...
const JobCard: React.FC<JobCardProps> = ({ title, customer, pet, freelancer, time, location, status, price }) => {
  ...
  const getStatusLabel = (jobStatus: string) => {
      ...
      <div className="flex items-start justify-between mb-4 pb-4 ">
        <div className="flex-1 min-w-0">
          ...
          <div className={`text-sm mt-1 flex items-center ${theme === 'dark' ? 'text-gray-300' : 'text-gray-500'}`}>
            <span className="font-medium">{t('Trạng thái:', 'Status:')}</span>
  ...
  <DetailItem icon={<AiOutlineUser />} label={t('Khách Hàng','Customer')} value={customer} />
  <DetailItem icon={<FaPaw />} label={t('Thú Cưng','Pet')} value={pet} />
  <DetailItem icon={<AiOutlineTeam />} label={t('Freelancer','Freelancer')} value={freelancer} />
  <DetailItem icon={<AiOutlineClockCircle />} label={t('Thời Gian','Time')} value={time} />
  <DetailItem icon={<AiOutlineEnvironment />} label={t('Địa Điểm','Location')} value={location} />
  <DetailItem icon={<AiOutlineDollarCircle />} label={t('Giá','Price')} value={price} />
      </div>
