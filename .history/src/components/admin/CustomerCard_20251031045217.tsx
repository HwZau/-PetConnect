...
const CustomerCard: React.FC<CustomerCardProps> = ({
  ...
  // LOGIC CHO NÚT BẤM
  let buttonLabel = actionLabel || t('Xem Chi Tiết', 'View Details');
  ...
  if (isInactive) {
    buttonLabel = t('Liên hệ lại', 'Contact');
  ...
  return (
    <div className={`${theme === 'dark' ? 'bg-gray-800' : 'bg-white'} rounded-2xl p-5 shadow-xl`}>
      ...
      {(pet || bookingCount !== undefined || totalSpent || lastBooking || region) && (
          ...
          {pet && <DetailItem icon={<FaPaw />} label={t('Thú cưng', 'Pet')} value={pet} />}
          {bookingCount !== undefined && <DetailItem icon={<AiOutlineCalendar />} label={t('Số lần đặt', 'Booking Count')} value={`${bookingCount} ${t('lần','times')}`} />}
          {totalSpent && <DetailItem icon={<AiOutlineDollarCircle />} label={t('Tổng chi tiêu', 'Total Spent')} value={totalSpent} />}
          {lastBooking && <DetailItem icon={<AiOutlineClockCircle />} label={t('Lần cuối đặt', 'Last Booking')} value={lastBooking} />}
