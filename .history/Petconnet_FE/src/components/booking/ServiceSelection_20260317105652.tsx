  ...
  const serviceOptions = freelancer?.services
    ? freelancer.services
        .map((service: any, index: number) => {
          ...
          return {
            id: serviceId, // Prefer the backend _id, fall back to id
        ...
        .filter((service: { id?: string; name: string; description: string; price: number }) => service.id) // Only include services with valid IDs
    : ServiceManager.getServicesForSelect();

  // Debug: Log service options and selected IDs
  console.log('ServiceSelection Debug:', {
    freelancerId: freelancer?.id,
    serviceOptions: serviceOptions.map((s: { id?: string; name: string; description: string; price: number }) => ({ id: s.id, name: s.name })),
    selectedServiceIds,
    freelancerServices: freelancer?.services
  });
