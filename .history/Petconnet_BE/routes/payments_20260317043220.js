...
router.post('/create', auth, async (req, res) => {
  try {
    ...
    await payment.save();
    await payment.populate(['bookingId', 'userId']);
    ...
    // Create a QR payload string that can be scanned by MoMo / VNPAY apps
    // (For demo purposes, this is a simple formatted string)
    const qrData = `PAY|${methodDetails.name}|${methodDetails.account}|${methodDetails.recipientName}|${booking.totalAmount}|${payment.orderCode}`;

    console.log('Payment created successfully:', { paymentId: payment._id, method, amount: booking.totalAmount });
