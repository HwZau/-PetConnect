...
router.post('/create', auth, async (req, res) => {
  try {
    ...
    // Create VietQR format (standard Vietnam QR format)
    // Format: vietqr://transfer?bank=BANKCODE&account=ACCOUNT&accountName=NAME&amount=AMOUNT&description=DESC
    
    let qrData = '';
    
    if (method === 2) {
      // MoMo: vietqr://transfer?bank=MOMO&...
      qrData = `vietqr://transfer?bank=MOMO&account=${methodDetails.account}&accountName=${encodeURIComponent(methodDetails.recipientName)}&amount=${booking.totalAmount}&description=${encodeURIComponent(`Pet Connect - ${payment.orderCode}`)}`;
    } else if (method === 3) {
      // TPBank: vietqr://transfer?bank=TPBVN&...
      qrData = `vietqr://transfer?bank=TPBVN&account=${methodDetails.account}&accountName=${encodeURIComponent(methodDetails.recipientName)}&amount=${booking.totalAmount}&description=${encodeURIComponent(`Pet Connect - ${payment.orderCode}`)}`;
    }
