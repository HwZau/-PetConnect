...
interface EditServiceModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (serviceId: string, data: Omit<Service, "id">) => void;
  isLoading?: boolean;
  service: Service | null;
}

// Map category names to ServiceType enum (matching backend API)
const SERVICE_CATEGORIES = [
  { value: "0", label: "Grooming (Chăm sóc & Làm đẹp)" },
  { value: "1", label: "Training (Huấn luyện)" },
  { value: "2", label: "Walking (Dắt dạo)" },
  { value: "3", label: "Sitting (Trông giữ thú cưng)" },
];
...
const EditServiceModal = ({
  ...
  useEffect(() => {
    if (service) {
      // Convert type string to number for dropdown
      const typeValue =
