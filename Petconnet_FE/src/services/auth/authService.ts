...
export const authService = {
  ...
  async getProfile(): Promise<ApiResponse<User>> {
    ...
    if (basicProfile.success && basicProfile.data) {
      console.log("[authService.getProfile] Basic profile success");
      // Extract user from response.data.user
      const rawUser = (basicProfile.data as any).user || basicProfile.data;
      const normalizedUser = {
        ...rawUser,
        id:
          rawUser.id ||
          rawUser._id ||
          (rawUser._id?.toString ? rawUser._id.toString() : undefined),
      };
      return {
        success: true,
        data: normalizedUser,
        message: basicProfile.message || "Profile fetched successfully",
      };
    }
    ...
    if (freelancerProfile.success && freelancerProfile.data) {
      console.log("[authService.getProfile] Freelancer profile success");
      // Extract user from response.data.user
      const rawUser = (freelancerProfile.data as any).user || freelancerProfile.data;
      const normalizedUser = {
        ...rawUser,
        id:
          rawUser.id ||
      ...
      return {
        success: true,
        data: normalizedUser,
        message: freelancerProfile.message || "Profile fetched successfully",
      };
    }
