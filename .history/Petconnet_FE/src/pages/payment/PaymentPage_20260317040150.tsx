...
const PaymentPage = () => {
  ...
  const handlePayment = async () => {
    ...
    try {
      // Handle manual payment methods (bank transfer and MoMo)
      if (paymentMethod === "bank" || paymentMethod === "wallet") {
        const methodCode = paymentMethod === "bank" ? PaymentMethodCode.BANK_TRANSFER : PaymentMethodCode.MOMO;
        ...
        console.log("Creating manual payment with:", {
        ...
        const paymentRes = await paymentService.createPayment({
          ...
          description: `Thanh toán qua ${paymentMethod === "bank" ? "chuyển khoản ngân hàng" : "ví MoMo"}`,
        });

        // For manual payments, navigate to success page with instructions
        navigate("/payment-success", {
          state: {
            ...
            manualPayment: true,
            accountInfo: paymentMethod === "bank"
              ? { type: "TP Bank", number: "0834339521", name: "Nguyễn Văn A" }
              : { type: "MoMo", number: "0834339521", name: "Nguyễn Văn A" }
          },
        });
        return;
      }
