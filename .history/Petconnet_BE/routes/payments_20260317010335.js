...
router.post('/create', auth, async (req, res) => {
  try {
    ...
    const paymentMethods = {
      0: { name: 'PayOS', account: null },
      1: { name: 'TP Bank', account: '0834339521' },
      2: { name: 'MoMo', account: '0834339521' }
    };

    const methodDetails = paymentMethods[method] || paymentMethods[0];

    // Mock PayOS integration for development
    const mockPayOSUrl = `${process.env.FRONTEND_URL || 'http://localhost:5173'}/payment-success?bookingId=${bookingId}&paymentId=${payment._id}&status=success`;
    ...
    res.status(201).json({
      ...
      amount: booking.totalAmount,
      currency: 'VND',
      redirectUrl: mockPayOSUrl,
      url: mockPayOSUrl,
      paymentUrl: mockPayOSUrl
    });
  } catch (error) {
