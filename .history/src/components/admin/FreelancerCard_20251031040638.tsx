...
const FreelancerCard: React.FC<FreelancerCardProps> = ({
    ...
    // LOGIC CHO NÚT BẤM
    let buttonLabel = actionLabel || t('Xem Hồ Sơ', 'View Profile');
    ...
    if (isSuspended) {
        buttonLabel = t('Xem Lý Do', 'View Reason');
        ...
        <div className={`${theme === 'dark' ? 'bg-gray-800 text-gray-100' : 'bg-white text-gray-800'} rounded-2xl p-5 shadow-xl transition duration-300 hover:shadow-2xl`}>
            ...
            {(experience || rating !== undefined || jobsCompleted !== undefined || servicePrice || region) && (
                <div className={`space-y-2 pt-4 mt-4 border-t ${theme === 'dark' ? 'border-gray-700' : 'border-gray-100'}`}>
                    {experience && <DetailItem icon={<AiOutlineClockCircle />} label={t('Kinh nghiệm','Experience')} value={experience} />}
                    {rating !== undefined && <DetailItem icon={<AiOutlineStar />} label={t('Đánh giá','Rating')} value={`${rating}/5`} />}
                    {jobsCompleted !== undefined && <DetailItem icon={<AiOutlineTrophy />} label={t('Hoàn thành','Completed')} value={`${jobsCompleted} ${t('công việc','jobs')}`} />}
                    {servicePrice && <DetailItem icon={<AiOutlineDollarCircle />} label={t('Giá TB','Avg. Price')} value={servicePrice} />}
                    {region && <DetailItem icon={<AiOutlineEnvironment />} label={t('Khu Vực','Region')} value={region} />}
                </div>
            )}
